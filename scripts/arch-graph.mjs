...
if (process.argv.includes('--check')) {
  ...
  /*
   * ── WHAT THIS CHECK COMPARES, AND THE TWO THINGS IT DELIBERATELY IGNORES ──
   *
   * It guards the GRAPH: which packages exist, which layer each is in, and
   * which depends on which. That is what comes from `package.json`, and that is
   * what a drawing of the architecture can be wrong about.
   *
   * It ignores the DATE, which moves every midnight and says nothing.
   *
   * It ignores LINE COUNTS, and that one was learned the hard way. They are not
   * read from `package.json` at all — they are a secondary measure of the source
   * tree, so ANY edit to ANY file makes them move. The first version compared
   * them and went red immediately after an unrelated change to a page, which is
   * a checker that fires on every commit and therefore a checker somebody turns
   * off. `/learn/drift` is a page about exactly that failure, and this check was
   * committing it.
   *
   * The counts stay IN the artifact, because the page prints them and they are
   * regenerated by `pnpm arch:graph` like everything else. They are just not
   * something this command is willing to fail a build over.
   */
  const strip = (s) =>
    s
      .replace(/export const GENERATED_AT = "[^"]*";/, '')
      .replace(/"lines": \d+,/g, '');
