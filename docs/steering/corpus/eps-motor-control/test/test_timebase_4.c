/*
 * test_timebase_4.c — unit tests for SWC-PLT-020
 *
...
void test_Timebase_Put_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, Timebase_Put(NULL_PTR, NULL_PTR));
}

void test_Timebase_Put_is_symmetric(void)
{
    Timebase4In_t  in  = { 0 };
    Timebase4Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, Timebase_Put(&in, &out));
...
void test_Timebase_Get_holds_at_zero(void)
{
    Timebase4In_t  in  = { 0 };
    Timebase4Out_t out = { 0 };
