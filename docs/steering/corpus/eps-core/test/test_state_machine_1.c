/*
 * test_state_machine_1.c — unit tests for SWC-PLT-027
 *
...
void test_StateMachine_Update_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, StateMachine_Update(NULL_PTR, NULL_PTR));
}

void test_StateMachine_Update_saturates_at_limit(void)
{
    StateMachine1In_t  in  = { 0 };
    StateMachine1Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, StateMachine_Update(&in, &out));
...
void test_StateMachine_Flush_is_monotonic(void)
{
    StateMachine1In_t  in  = { 0 };
    StateMachine1Out_t out = { 0 };
...
void test_StateMachine_Get_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, StateMachine_Get(NULL_PTR, NULL_PTR));
}

void test_StateMachine_Get_holds_at_zero(void)
{
    StateMachine1In_t  in  = { 0 };
    StateMachine1Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, StateMachine_Get(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}
