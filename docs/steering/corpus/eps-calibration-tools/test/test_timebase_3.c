/*
 * test_timebase_3.c — unit tests for SWC-PLT-001
 *
...
void test_Timebase_Update_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, Timebase_Update(NULL_PTR, NULL_PTR));
}

void test_Timebase_Update_is_monotonic(void)
{
    Timebase3In_t  in  = { 0 };
    Timebase3Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, Timebase_Update(&in, &out));
...
void test_Timebase_Peek_is_monotonic(void)
{
    Timebase3In_t  in  = { 0 };
    Timebase3Out_t out = { 0 };
...
void test_Timebase_Reset_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, Timebase_Reset(NULL_PTR, NULL_PTR));
}

void test_Timebase_Reset_holds_at_zero(void)
{
    Timebase3In_t  in  = { 0 };
    Timebase3Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, Timebase_Reset(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}
...
void test_Timebase_Check_is_monotonic(void)
{
    Timebase3In_t  in  = { 0 };
    Timebase3Out_t out = { 0 };
