/*
 * test_ring_buffer_2.c — unit tests for SWC-PLT-023
 *
...
void test_RingBuffer_Flush_is_symmetric(void)
{
    RingBuffer2In_t  in  = { 0 };
    RingBuffer2Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, RingBuffer_Flush(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}

void test_RingBuffer_Update_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, RingBuffer_Update(NULL_PTR, NULL_PTR));
}

void test_RingBuffer_Update_holds_at_zero(void)
{
    RingBuffer2In_t  in  = { 0 };
    RingBuffer2Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, RingBuffer_Update(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}
