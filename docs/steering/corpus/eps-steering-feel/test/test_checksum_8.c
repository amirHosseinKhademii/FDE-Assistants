...
void test_Checksum_Flush_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, Checksum_Flush(NULL_PTR, NULL_PTR));
}

void test_Checksum_Flush_is_monotonic(void)
{
    Checksum8In_t  in  = { 0 };
    Checksum8Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, Checksum_Flush(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}
...
void test_Checksum_Calc_saturates_at_limit(void)
{
    Checksum8In_t  in  = { 0 };
    Checksum8Out_t out = { 0 };
...
void test_Checksum_Peek_rejects_null(void)
{
    TEST_ASSERT_EQUAL(E_NOT_OK, Checksum_Peek(NULL_PTR, NULL_PTR));
}

void test_Checksum_Peek_is_monotonic(void)
{
    Checksum8In_t  in  = { 0 };
    Checksum8Out_t out = { 0 };
    TEST_ASSERT_EQUAL(E_OK, Checksum_Peek(&in, &out));
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.0f, out.out);
}
